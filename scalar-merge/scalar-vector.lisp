; This file contains the transformation and proof of skipping simulation theorem.
(in-package "ACL2S")
...
; define a new run relation to reason about the upto-3-step-relation
...
(defthm run-expanded-equivalent-isa-run-n
...
(defthm run-expanded-exist-equivalent
  (implies (spec-run-expanded-exist s u)
         (spec-step+ s u))
  :hints (("goal" :use ((:instance run-expanded-equivalent-isa-run-n (n (spec-run-expanded-exist-witness s u)))
...
(defthm get-scaler-inst-contract
  (implies (vecinstp inst)
           (imemp (scalarize inst)))
  :hints (("goal" :in-theory (enable vecinstp instp)))
  :rule-classes :forward-chaining)


(defthm scalarize-contract
  (implies (and (vecimemp vecimem)
                (integerp k))
           (imemp (scalarize (nth k vecimem))))
  :hints (("goal" :in-theory (enable vecinstp instp))))
...
(defthmd get-num-inst-decreases
  ...
  :rule-classes :linear)
  ;; :hints (("goal" :in-theory (disable flatten-len-get-num-inst
  ;;                                     count-num-scaler-inst))
  ;;         ("subgoal 3" :use (:instance count-num-scaler-inst->0 (i (nth k vecimem))))
  ;;         ("subgoal 2" :use (:instance count-num-scaler-inst->0 (i (car vecimem)))))
  ;; :rule-classes :linear)
...
;; (defthm mget-pc-natp
;;   (implies (vecisa-statep s)
;;            (natp (mget :prc s)))
;;   :hints (("goal" :in-theory (enable vecisa-statep)))
;;   :rule-classes :forward-chaining)

;; vecimem is mapped to (flatten vecimem)  by the refinement map. I need some
;; relation that relates the instruction executed in the vector machine at
;; vecpc and the one pointed by the get-num-inst in the flattened vecimem. Infact, I
;; need to prove that the instruction is same in case the next instruction to
;; be executed in vector machine is a scaler instruction and similarly if the
;; next is a vector instruction then its decomposition is two consecutive
;; scalar instruction. These obligations are captured in lemma 5,6,7
(encapsulate
 ()
 (local 
  (defthm lemma-5-main
...
(defthmd vecinst-or-inst-vecimem 
  (implies (and (vecimemp v)
                (natp k)
                (nth k v)
                (instp (nth k v)))
           (not (vecinstp (nth k v))))
  :hints (("goal" :in-theory (e/d (vecinstp instp) ))))
...
(defthmd case-exhaustive
  (implies (and (vecimemp v)
                ...
                (not (case-1-inst k v)))
           (case-2-inst k v))
  :hints (("subgoal 1" :use ((:instance vecinst-or-inst-vecimem)))))
...
(defthm impl-simulates-spec
  ...
  :hints (("goal" :use ((:instance finite-step-implies-exists 
                        ...
                        (:instance impl-simulates-vecinst-run-lemma)))))#|ACL2s-ToDo-Line|#
